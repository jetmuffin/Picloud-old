...
public class HbaseOperationImpl implements IHbaseOperation {
	 ...
	 */
	@Override
	public ResultScanner getOtherImages(String spaceId, String imageName,
			int num) {
		try {
			HTable table = new HTable(mConfiguration, "cloud_image");
			List<Filter> filters = new ArrayList<Filter>();
			...
			Filter filter1 = new SingleColumnValueFilter(Bytes.toBytes("attr"),
					Bytes.toBytes("name"), CompareOp.NOT_EQUAL,
					Bytes.toBytes(imageName));
			filters.add(filter1);
	...
	public ResultScanner queryLimitImage(String uid, String sTime, String eTime) {
		try {
			...
			Filter filter1 = new SingleColumnValueFilter(Bytes.toBytes("attr"),
					Bytes.toBytes("createTime"), CompareOp.GREATER_OR_EQUAL,
					Bytes.toBytes(sTime));
			filters.add(filter1);

			Filter filter2 = new SingleColumnValueFilter(Bytes.toBytes("attr"),
					Bytes.toBytes("createTime"), CompareOp.LESS_OR_EQUAL,
					Bytes.toBytes(eTime));
			filters.add(filter2);
			...
			Filter filter4 = new SingleColumnValueFilter(Bytes.toBytes("var"),
					Bytes.toBytes("status"), CompareOp.NOT_EQUAL,
					Bytes.toBytes("deleted"));
			filters.add(filter4);
	...
	public ResultScanner logPage(String uid, String rowkey, int num) {
		try {
			...
			Filter rf = new RowFilter(CompareOp.GREATER_OR_EQUAL,
	...
	public ResultScanner imagePageByTime(String time, String uid, int num) {
		try {
			...
			Filter filter1 = new SingleColumnValueFilter(Bytes.toBytes("attr"),
					Bytes.toBytes("createTime"), CompareOp.LESS,
					Bytes.toBytes(time));
			filters.add(filter1);
	 ...
	 */
	@Override
	public ResultScanner imagePageByKey(String uid, String key, String space,
			int num) {
		try {
			HTable table = new HTable(mConfiguration, "cloud_image");
			List<Filter> filters = new ArrayList<Filter>();
